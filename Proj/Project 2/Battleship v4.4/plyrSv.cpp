...
void plyrSv::PSetUp (int maxSpc,string PName,string b[][9],shipDat *PShips)
{
    ...
    BSV *sif = new BSV();                       //New utility class (should run basic zero-ing constructor)
...
{
    bool *cmpFlg = new bool; *cmpFlg=false;
    BSAI *bot = new BSAI();
    BSV *sif = new BSV();                       //siffy takes the place of cnt
    ...
    {    
            for (; turn<=tMax; )  //Due to save/load features, now relies on minimal data.  Counter increments at bottom before game saves
            {
                ...
                bool *ugh=new bool;
                Shoot(sif->useS(),BrdDat.B2S,BrdDat.B1G,P1Pts,*ugh);
                delete ugh;
                cout<<endl;
                ...
                else //Endgame Check Fails, Run Player 2 Turn
                {
                        ...
                        {   cout<<"----------------------------------"<<endl;
                            cout<<"The Computer will now make a guess"<<endl;
                            cout<<"----------------------------------"<<endl;
                            ...
                            sophie->flipB();
                                ...
                                do
                                {  
                                sophie->sifS2S(bot->AIguess(*cmpFlg,rand(),rand()));
                                sophie->sifS2I(sophie->useS().substr(1,1));
                                sophie->sifS2S(sophie->useS().substr(0,1));
                                }while(sophie->useS()>"I" || sophie->useS()<"A" || sophie->useI()>9 || sophie->useI()<1);
                                for(int u=0;u<=sif->useI();u++)
                                {
                                if((sophie->useS()+sophie->sndI2S())==noregus[u]){sophie->flipB();u=sif->useI();} //Ensures the computer does not guess the same thing twice
                                else if(u==sif->useI()){sophie->flipF();}
                                }    
                                }
                            sif->sifS2S(sophie->useS()+sophie->sndI2S()); //concatenate string for guess
                            cout<<"---------------------------"<<endl;
                        ...
                        cout<<"yes1"<<endl;
                        Shoot(sif->useS(),BrdDat.B1S,BrdDat.B2G,P2Pts,*cmpFlg);
                        cout<<"yes2"<<endl;
                        ...
                        else{cout<<"save issues"<<endl;MakSave(file);cout<<"|Autosaved|"<<endl;}          //If game will continue, send data to save file
                }
                cout<<"---==Turn End==---"<<endl;
...
void plyrSv::Shoot (string user,string b[][9],string g[][9],short &pts,bool &x)
{
...
void plyrSv::destroy (binXfer *todel,int Z9,char dType)
{
    int Zmod = 0;
    if(dType=='B'){Zmod=Z9*4;}  //Determine size of binXfer to create based on file name
    else if(dType=='C'){Zmod=(Z9+6);}
...
void plyrSv::Bin2Brd(binXfer *dater)
    ...
    for(int p=0;p<BrdDat.Z9;p++)   
     {
        ...
        char ltrB, numB;
        ltrB=dater[Xcnt].ltr[q];
        numB=dater[Xcnt].num[q];
...
void plyrSv::LdSave (plyrSv &gamDat,fstream &file)
{
    ...
    destroy(lore,BrdDat.Z9,'C');                   //Clean up nasty memory
...
{cout<<"in make save"<<endl;
    ...
    cout<<"not loop"<<endl;
    brds=Brd2Bin();         //Converts game data to binXfer structure
    cout<<"now what"<<endl;
    binWrt(brds,file,BrdDat.Z9,"brdSv");           //Sends binXfer structure to file
        cout<<"it's delete"<<endl;
