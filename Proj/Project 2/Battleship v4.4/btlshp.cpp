...
{   cout<<"--------Player Scores---------------------------------------------"<<endl;
    cout<<"Name"<<"\t"<<"\t"<<"W/L"<<"\t"<<"Pts"<<"\t"<<"Turns"<<"\t"<<"Games"<<"\t"<<"W/L %"<<"\t"<<"Accuracy"<<endl;
            for(int h=0;h<(Scrs.size());h++)
            {
                cout<<Scrs[h].Name<<"\t";
                if(Scrs[h].Name.length()<8){cout<<"\t";}
                cout<<Scrs[h].Vs<<"\t"<<Scrs[h].Ps<<"\t"<<Scrs[h].Ts<<"\t"<<Scrs[h].Gs<<"\t"<<fixed<<setprecision(1)<<ratio(Scrs[h].Vs,Scrs[h].Gs)<<"%\t"<<ratio(Scrs[h].Ps,Scrs[h].Ts)<<"%"<<endl;
            }cout<<"------------------------------------------------------------------"<<endl;
}
...
break;}
        case '?':{
            cout<<"AI Ship Finder Example (more entertaining than the game!)"<<endl;
            cout<<"Destroys a 5-space ship or gets lost trying!"<<endl;
            cout<<"---------------------------------------------------------"<<endl;
            ...
            BSAI bot = BSAI();
            bool y = false;
            cout<<bot.AIguess(y,rand(),rand())<<endl;
            cout<<endl;
            cout<<"----"<<endl;
            y=true;
                 cout<<"1 - look around"<<endl;       
            do
            {  cout<<"1 valid loop"<<endl;
            brf=bot.AIguess(y,rand(),rand());
            buff=brf.substr(0,1);
            stringstream a(brf.substr(1,1));
            a>>bff;
            cout<<"Result: |"<<brf<<"|"<<endl;
            }while(brf.length()!=2 || buff>"I" || buff<"A" || bff>9 || bff<1);
            
            y=false;
            cout<<"--new test block---------"<<endl;
            cout<<"2 - look around"<<endl;
            do
            {  cout<<"2 valid loop"<<endl;
            brf=bot.AIguess(y,rand(),rand());
            buff=brf.substr(0,1);
            stringstream a(brf.substr(1,1));
            a>>bff;
            ...
            y=true;
                 cout<<"3 found it - go down"<<endl;       
            do
            {  cout<<"3 valid loop"<<endl;
            brf=bot.AIguess(y,rand(),rand());
            buff=brf.substr(0,1);
            stringstream a(brf.substr(1,1));
            a>>bff;
            ...
            y=true;
                 cout<<"4 - keep going down"<<endl;       
            do
            {  cout<<"4 valid loop"<<endl;
            brf=bot.AIguess(y,rand(),rand());
            buff=brf.substr(0,1);
            stringstream a(brf.substr(1,1));
            a>>bff;
            ...
            y=true;
                    cout<<"5 - keep going down"<<endl;    
            do
            {  cout<<"5 valid loop"<<endl;
            brf=bot.AIguess(y,rand(),rand());
            buff=brf.substr(0,1);
            stringstream a(brf.substr(1,1));
            a>>bff;
            cout<<"Result: |"<<brf<<"| is a miss, change direction"<<endl;
            }while(brf.length()!=2 || buff>"I" || buff<"A" || bff>9 || bff<1);
                        cout<<"--new test block---------"<<endl;

            y=false;
                   cout<<"6 - change direction"<<endl;    
            do
            {  cout<<"6 valid loop"<<endl;
            brf=bot.AIguess(y,rand(),rand());
            buff=brf.substr(0,1);
            stringstream a(brf.substr(1,1));
            a>>bff;
            ...
            y=true;
            cout<<"7 - hit, keep going"<<endl;    
            do
            {  cout<<"7 valid loop"<<endl;
            brf=bot.AIguess(y,rand(),rand());
            buff=brf.substr(0,1);
            stringstream a(brf.substr(1,1));
            a>>bff;
            ...
            y=false;
                   cout<<"8 - finished - reset"<<endl;    
            do
            {  cout<<"8 valid loop"<<endl;
            brf=bot.AIguess(y,rand(),rand());
            buff=brf.substr(0,1);
            stringstream a(brf.substr(1,1));
            a>>bff;
            ...
            //for(int i=0;i<25;i++){cout<<bot.AIguess(y,rand(),rand())<<endl;}
