...
public:
    bool operator<(const shipDat x){if(size<x.size){return true;}else{return false;}}
    bool operator>(const shipDat x){if(size>x.size){return true;}else{return false;}}
    bool operator==(int x){if(size==x){return true;}else{return false;}}
    ...
    shipDat cpyShp(shipDat x){shipDat *cpy = new shipDat();*cpy = x;return *cpy;}
